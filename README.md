...
# Apply on STOP Button (Enable / Disable)<br>
In Blender, characters that do not have any keyframes on the timeline will always return to their Rest Pose when scrubbing the timeline.<br>
This is Blender's default behavior.<br>
If you scrub the timeline while synchronization is active, the character may instantly switch between the Rest Pose and the current synchronized pose.<br>
This appears as a flickering effect.<br>
The **Apply on STOP** option determines whether **Apply** is executed automatically once when synchronization is stopped.<br>

**Enabled:**<br>
Every time synchronization is stopped, **Apply** is executed automatically and keyframes are inserted.<br>

**Disabled:**<br>
No action is performed.<br>

Once keyframes have been created, it is recommended to disable this option.<br>
If it remains enabled, additional keyframes will be inserted every time synchronization is stopped, which may create unwanted keyframes in your animation.<br>
...
# Apply on STOP ボタンの有効/無効 <br>
タイムライン上にキーフレームが打たれていないキャラクターはタイムラインシーク時にレストポーズを取る<br>
Blenderの絶対的な仕様があります。<br>
その状態でタイムライン操作をするとレストポーズと現在のポーズが瞬間的に切り替わる現象が発生します。<br>
ちらつく様な現象です。このボタンの有効/無効はSTOP時に一度だけApplyを実行するかしないかの設定です。<br>
有効時：STOPする度にApplyを自動実行してキーフレームを打ちます。<br>
無効時：何もしません。<br>
一度キーフレームが打たれたら、その後は無効にすることをお勧めします。<br>
有効のままですとSTOPする都度に意図しない場所にキーフレームが打たれますので注意が必要です。<br>
